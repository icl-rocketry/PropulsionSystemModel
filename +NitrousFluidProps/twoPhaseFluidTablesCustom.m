function varargout = twoPhaseFluidTablesCustom(varargin)
...
try
    ...
    %Only ever called with Q = 0 or Q = 1
    isLiquid = abs(q) < 0.001;
    assert(isLiquid || abs(1-q) < 0.001); %Assert Q was 0 or 1, give or take
    
    %Fetch mu and K from NIST tables
    if(isLiquid)
        mu = NitrousFluidProps.NistNitrous.getLiquidViscosity(T,p_Pa);
        k = NitrousFluidProps.NistNitrous.getLiquidThermalConductivity(T,p_Pa);
    else %Is gas
        mu = NitrousFluidProps.NistNitrous.getGasViscosity(T,p_Pa);
        k = NitrousFluidProps.NistNitrous.getGasThermalConductivity(T,p_Pa);
    end
    ...
    if(Pr < 1e-9) %Catch error at very edge of domain for vapor
       Pr = 1e-9; 
    end
catch errCoolProp
...
try
    ...
    if( T > 310) %If above critical pressure, we are a gas. Coolprop doesn't like asking for PVap above this
        PVap = Inf;
    else
        try
            PVap = coolpropFun('P', 'T', T, 'Q', 0);
        catch %PVap sometimes fails for extreme T
            %Use polynomial fit for vapour pressure instead of coolprop
